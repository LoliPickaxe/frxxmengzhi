...
public class BlockLingLiGenerator extends Block implements ITileEntityProvider {
    ...
    public BlockLingLiGenerator(int tier) {
        super(Material.IRON);
        this.tier = tier;
        setHardness(2.5F);
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (!world.isRemote) {
            player.openGui(FanRenXiuXianMengZhi.INSTANCE, LingLiGuiHandler.GUI_ID_GENERATOR, world, pos.getX(), pos.getY(), pos.getZ());
        }
        return true;
    }
    ...
    public void getDrops(NonNullList<ItemStack> drops, net.minecraft.world.IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
        ...
        if (tile instanceof TileLingLiGenerator && !drops.isEmpty()) {
            TileLingLiGenerator generator = (TileLingLiGenerator) tile;
            boolean hasEnergy = generator.getEnergyStored() > 0;
            boolean hasFuel = !generator.fuelHandler.getStackInSlot(0).isEmpty();
            boolean hasCharge = !generator.chargeHandler.getStackInSlot(0).isEmpty() || !generator.chargeOutputHandler.getStackInSlot(0).isEmpty();
            if (hasEnergy || hasFuel || hasCharge) {
                ...
                if (hasEnergy) {
                    tag.setInteger(ENERGY_TAG, generator.getEnergyStored());
                }
    ...
    public TileEntity createNewTileEntity(World world, int meta) {
        return TileLingLiGenerator.create(tier);
    }
